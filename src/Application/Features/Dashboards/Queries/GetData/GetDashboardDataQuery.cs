...
namespace DynaCore.Application.Features.Dashboards.Queries.GetData
{
    ...
    internal class GetDashboardDataQueryHandler : IRequestHandler<GetDashboardDataQuery, Result<DashboardDataResponse>>
    {
        ...
        public async Task<Result<DashboardDataResponse>> Handle(GetDashboardDataQuery query, CancellationToken cancellationToken)
        {
            var response = new DashboardDataResponse
            {
                ...
                VendorCount = await _unitOfWork.Repository<Vendor>().Entities.CountAsync(cancellationToken),
            ...
            for (int i = 1; i <= 12; i++)
            {
                ...
                brandsFigure[i - 1] = await _unitOfWork.Repository<Vendor>().Entities.Where(x => x.CreatedOn >= filterStartDate && x.CreatedOn <= filterEndDate).CountAsync(cancellationToken);
            ...
            response.DataEnterBarChart.Add(new ChartSeries { Name = _localizer["Vendors"], Data = brandsFigure });
